const char* SendHTML = R"rawLiteral(
<!DOCTYPE html>
<html>
<head>
    <title>Encoder Home Page</title>
</head>
<body text="CACACA" style="background-color:#2f2f2f;">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <h1 style="font-size:3em">Setting up the Encoder<br></h1>
    <p>
        1. Slide the base onto the front of the safe<br>
        2. Slide the encoder onto the metal rod and make sure it's centered on the dial before tightening the screw onto the rod.<br>
        3. If there's not enough friction to spin the encoder then you may need to simply tape it onto the dial. It doesn't take much to get it spinning well with the dial.<br><br>
    </p>
    <h1 style="font-size:3em">Current number<br>
    <label id="position">0</label></h1><br><br>
    Graph<br>
    <canvas id="graph" style="width:device-width;max-width:800px"></canvas><br>
    <label for="testing">Number: </label><input type="text" id="testing" name="contactpoint" size="5"><br>
  ...
  fetch('/plot?testing=' + encodeURIComponent(testing) + '&rcontactpoint=' + encodeURIComponent(rcontactpoint) + '&lcontactpoint=' + encodeURIComponent(lcontactpoint))
      ...
      .then(data => {
          ...
          if (parts.length == 3) {
            ...
            if (direction == "R") {
              graph.data.datasets[0].data.push({x: x, y: y});
            }
...
document.getElementById("savegraph").addEventListener("click", function () {
    ...
    link.href = image;
    link.download = "graph.png"; // File name for the downloaded image
